...
public class AligntoCoral extends Command {
    ...
    public AligntoCoral(Supplier<Double> translateX, Supplier<Double> translateY) {
        ...
        getTargetRotation = () -> {
            double deltaX = drivetrain.getPose().getX() - LimelightHelpers.getTX(Constants.VisionConstants.kLimelightCoralName);
            double deltaY = drivetrain.getPose().getY() - LimelightHelpers.getTY(Constants.VisionConstants.kLimelightCoralName);
            double angleRadians = Math.atan2(deltaY, deltaX);
            return Rotation2d.fromDegrees(Math.toDegrees(angleRadians));
        };
    }
